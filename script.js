...
function initializeForm() {
    ...
    bookingForm.addEventListener('submit', function(e) {
        ...
        if (typeof grecaptcha === 'undefined') return showNotification('Erreur de configuration de la s̩curit̩.', 'error');
        ...
        grecaptcha.ready(() => {
            grecaptcha.execute('6LcYThAsAAAAAOV055t1Nvd5Uo94kcTmPUBd-cmq', {action: 'submit'}).then(async (recaptchaToken) => {
                ...
                try {
                    ...
                    showNotification('Votre demande a ̩t̩ envoy̩e avec succ̬s !');
                    bookingForm.reset();
                    fetchAndInitializeDatepicker();
                } catch (error) {
...
function initializeSubscriptionForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const result = await response.json();
            if (!response.ok) throw new Error(result.error || 'Une erreur est survenue.');
            messageDiv.textContent = 'Demande envoy̩e ! Nous vous contacterons bient̫t.';
