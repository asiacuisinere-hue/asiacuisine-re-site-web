...
serve(async (req) => {
  ...
  try {
    ...
    if (companyError) throw new Error(`Erreur récupération infos entreprise: ${companyError.message}`);
    if (!companySettingsArray || companySettingsArray.length === 0) {
      throw new Error('Informations entreprise non trouvées. Veuillez remplir les paramètres.');
    }
    ...
    if (quoteError) throw new Error(`Erreur sauvegarde devis: ${quoteError.message}`);
    ...
    if (itemsError) throw new Error(`Erreur sauvegarde lignes: ${itemsError.message}`);

    // --- 4. Générer le PDF ---
    const pdfBytes = await generatePDF(newQuote, customer, items, total, companySettings);

    // --- 5. Envoyer l'email via Resend ---
    const customerEmail = customer.type === "client" ? customer.email : customer.contact_email;
    const customerName = customer.type === "client" ? `${customer.first_name || ""} ${customer.last_name}`.trim() : customer.nom_entreprise;

    if (customerEmail) {
      await sendEmailWithResend(resendApiKey, customerEmail, customerName, documentNumber, pdfBytes, companySettings);
    }
    ...
    return new Response(pdfBytes, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="devis-${documentNumber.replace(/_/g, '-')}.pdf"; filename*=UTF-8''${encodeURIComponent(`devis-${documentNumber}.pdf`)}`,
...
// --- Génération du PDF ---
async function generatePDF(quote: any, customer: any, items: any[], total: number, companySettings: any): Promise<Uint8Array> {
  ...
  if (customer.address) page.drawText(customer.address || '', { x: 330, y: y - 37, size: 9, font, color: gray });
  ...
  page.drawText(companySettings.payment_conditions || "Conditions : Paiement à réception", { x: 300, y: y - 15, size: 9, font, color: gray });
  page.drawText(companySettings.payment_methods || "Moyens : Virement, Espèces, CB", { x: 300, y: y - 28, size: 9, font, color: gray });
