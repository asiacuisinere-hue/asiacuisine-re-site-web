...
async function generateInvoicePDF(invoice, companySettings) {
    ...
    if (invoice.items && Array.isArray(invoice.items)) {
        ...
        for (const item of invoice.items) {
            ...
            y -= 25;
            rowIndex++;
            if (y < 150) break; // Leave space for totals and footer
        }
    }
    
    y -= 15; // Space after items
    ...
    y -= 80; // Create space for the footer notes, similar to the quote
...
serve(async (req) => {
  ...
  try {
    ...
    if (invoiceId) {
        ...
        const { data: demandeData, error: demandeError } = await supabase.from("demandes").select(`*, clients(*), entreprises(*)`).eq("id", demandeId).single();
        ...
        if (existingInvoice) {
            ...
            if (currentDemande.type === 'COMMANDE_MENU') {
                ...
                if (currentDemande.type === 'RESERVATION_SERVICE') { serviceName = 'Réservation de service'; }
                else if (currentDemande.type === 'CUISINE_DOMICILE') { serviceName = 'Cuisine chez le client'; }
                else { serviceName = currentDemande.service_type || currentDemande.type || 'Service'; }
                items = [{ name: serviceName, description: currentDemande.details_json?.description || currentDemande.description || '', quantity: 1, unit_price: servicePrice }];
            }
        ...
        invoice.clients = currentDemande.clients;
        invoice.entreprises = currentDemande.entreprises;
    }
    ...
    await sendEmailWithResend(resendApiKey ?? "", customerEmail, customerName, invoice.document_number, pdfBytes, companySettings);
    ...
    return new Response(pdfBytes, { 
        headers: { ...corsHeaders, "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="facture-${invoice.document_number}.pdf"` }
    });
