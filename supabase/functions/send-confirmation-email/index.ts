...
serve(async (req) => {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 12px; padding: 30px;">
          ...
          <p>Bonne nouvelle ! Le Chef a bien reçu votre demande et elle est en cours de traitement.</p>
          ...
          <p>Vous pouvez suivre l'évolution de votre dossier et accéder à vos documents (devis, facture) à tout moment en cliquant sur le bouton ci-dessous :</p>
    ...
    // 3. Update Demande Status to 'Nouvelle' (Official)
    await supabase.from('demandes').update({ status: 'Nouvelle' }).eq('id', demandeId);
