document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateDisplay() {
        ...
        // Chaque 10% de slider ajoute/retire 10% des macros de base
        const proteinModifier = 1 + (proteinAdjustment / 100);
        const carbsModifier = 1 + (carbsAdjustment / 100);

        const finalCalories = Math.round(recipe.baseCalories * proteinModifier * carbsModifier);
        ...
        const finalFat = Math.round(recipe.baseFat * proteinModifier); // Simplification: la graisse suit les protéines
    ...
    function renderRecipeOptions() {
        ...
        document.querySelectorAll('input[name="base-recipe"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                // Réinitialiser les sliders lors du changement de recette
                proteinSlider.value = 0;
                carbsSlider.value = 0;
                proteinAdjustment = 0;
                carbsAdjustment = 0;
                updateDisplay();
                renderRecipeOptions(); // Re-render pour mettre à jour les styles
            });
        });
    }
    ...
    requestButton.addEventListener('click', () => {
        ...
        const subject = `Demande de plat personnalisé : ${recipe.name}`;
        const body = `Bonjour,

Je serais intéressé(e) par la configuration de plat suivante :
